...
| **Item-Gen-Agent** | 동적 문항 생성 (LLM 기반, MVP 2.0부터 RAG 통합) | 1~n차 문항 생성, 난이도 조정 |
...
| **REQ-302** | 생성된 문항은 다음을 포함해야 한다: <br> - 유형: 객관식, OX, 주관식 혼합 <br> - 카테고리: 사용자의 관심분야 반영 <br> - 난이도: 자기평가 수준 반영 <br> - 생성: LLM이 선택된 관심분야와 난이도를 기반으로 생성 | **M** |
| **REQ-303** | (MVP 2.0) RAG 소스 메타(문헌명, URL 해시, 버전, 타임스탬프)를 저장하여 추적성을 확보해야 한다. | **S** |
...
| REQ ID | 요구사항 | 우선순위 |
|--------|---------|---------|
| **REQ-501** | (MVP 2.0) RAG 소스 메타(문헌명, URL 해시, 버전, 타임스탐프)를 저장하여 추적 가능해야 한다. | **M** |
