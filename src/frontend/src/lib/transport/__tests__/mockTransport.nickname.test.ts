...
const postPrivateAuth = <T>(url: string, data?: any) =>
  mockTransport.post<T>(url, data, PRIVATE_AUTH_CONFIG)
...
describe('Mock Transport - Nickname Validation (REQ-F-A2-5)', () => {
  ...
  describe('1. Length Validation Tests', () => {
    test('POST /profile/nickname/check - 닉네임 너무 짧음 (2자)', async () => {
      await expect(
        postPrivateAuth('/profile/nickname/check', { nickname: 'ab' })
    ...
    test('POST /profile/nickname/check - 닉네임 너무 김 (31자)', async () => {
      ...
      await expect(
        postPrivateAuth('/profile/nickname/check', { nickname: longNickname })
      ).rejects.toThrow('닉네임은 30자 이하여야 합니다.')
    })

    test('POST /profile/nickname/check - 유효한 길이 (3자)', async () => {
      const response = await postPrivateAuth('/profile/nickname/check', { nickname: 'abc' })
    ...
    test('POST /profile/nickname/check - 유효한 길이 (30자)', async () => {
      ...
      const response = await postPrivateAuth('/profile/nickname/check', { nickname: validNickname })
      expect(response).toHaveProperty('available')
    })
  })
  ...
  describe('2. Format Validation Tests', () => {
    test('POST /profile/nickname/check - 특수문자 포함 (@)', async () => {
      await expect(
        postPrivateAuth('/profile/nickname/check', { nickname: 'name@email' })
      ).rejects.toThrow('닉네임은 영문자, 숫자, 언더스코어만 사용 가능합니다.')
    })

    test('POST /profile/nickname/check - 특수문자 포함 (-)', async () => {
      await expect(
        postPrivateAuth('/profile/nickname/check', { nickname: 'my-name' })
      ).rejects.toThrow('닉네임은 영문자, 숫자, 언더스코어만 사용 가능합니다.')
    })
    ...
    test('POST /profile/nickname/check - 유효한 형식 (영문+숫자+언더스코어)', async () => {
      const response = await postPrivateAuth<NicknameCheckResponse>('/profile/nickname/check', { nickname: 'player_123' })
      expect(response).toHaveProperty('available')
      expect(response.available).toBe(true)
    })

    test('POST /profile/nickname/check - 유효한 형식 (언더스코어만)', async () => {
      const response = await postPrivateAuth('/profile/nickname/check', { nickname: 'my_name_here' })
      expect(response).toHaveProperty('available')
    })
  })
  ...
  describe('3. Banned Words Validation Tests (REQ-F-A2-5 핵심)', () => {
    ...
    test('POST /profile/nickname/check - 여러 금칙어 테스트', async () => {
      ...
      for (const word of bannedWords) {
        await expect(
          postPrivateAuth('/profile/nickname/check', { nickname: word })
  ...
  describe('4. Integration Tests', () => {
    test('POST /profile/nickname/check - 유효한 닉네임 + 중복 없음', async () => {
      const response = await postPrivateAuth('/profile/nickname/check', { nickname: 'validuser' })
    ...
    test('POST /profile/nickname/check - 유효한 닉네임 + 중복 있음', async () => {
      const response = await postPrivateAuth('/profile/nickname/check', { nickname: 'mockuser' })
    ...
    test('POST /profile/nickname/check - 유효한 닉네임 + 중복 있음 (existing_user)', async () => {
      const response = await postPrivateAuth<NicknameCheckResponse>('/profile/nickname/check', { nickname: 'existing_user' })
  ...
  describe('5. Register Endpoint Tests', () => {
    test('POST /profile/register - 금칙어 등록 시도 (admin)', async () => {
      await expect(
        postPrivateAuth('/profile/register', { nickname: 'admin' })
      ).rejects.toThrow("'admin'은(는) 사용할 수 없는 닉네임입니다. 다른 닉네임을 선택해주세요.")
    })

    test('POST /profile/register - 금칙어로 시작하는 닉네임 등록 시도', async () => {
      await expect(
        postPrivateAuth('/profile/register', { nickname: 'bot_user' })
      ).rejects.toThrow('닉네임에 사용할 수 없는 단어가 포함되어 있습니다. 다른 닉네임을 선택해주세요.')
    })

    test('POST /profile/register - 너무 짧은 닉네임 등록 시도', async () => {
      await expect(
        postPrivateAuth('/profile/register', { nickname: 'ab' })
      ).rejects.toThrow('닉네임은 3자 이상이어야 합니다.')
    })

    test('POST /profile/register - 특수문자 포함 닉네임 등록 시도', async () => {
      await expect(
        postPrivateAuth('/profile/register', { nickname: 'name@123' })
      ).rejects.toThrow('닉네임은 영문자, 숫자, 언더스코어만 사용 가능합니다.')
    })

    test('POST /profile/register - 유효한 닉네임 등록 성공', async () => {
      const response = await postPrivateAuth<NicknameRegisterResponse>('/profile/register', { nickname: 'newuser123' })
  ...
  describe('6. Error Message Clarity Tests (REQ-F-A2-5 Acceptance)', () => {
    test('금칙어 거부 시 명확한 사유 제공 - exact match', async () => {
      try {
        await postPrivateAuth('/profile/nickname/check', { nickname: 'admin' })
    ...
    test('금칙어 거부 시 명확한 사유 제공 - starts with', async () => {
      try {
        await postPrivateAuth('/profile/nickname/check', { nickname: 'moderator_main' })
        throw new Error('Should have thrown validation error')
      } catch (error) {
        const errorMessage = (error as Error).message
    ...
    test('형식 오류 시 명확한 사유 제공', async () => {
      try {
        await postPrivateAuth('/profile/nickname/check', { nickname: 'my-name' })
        throw new Error('Should have thrown validation error')
      } catch (error) {
        const errorMessage = (error as Error).message
